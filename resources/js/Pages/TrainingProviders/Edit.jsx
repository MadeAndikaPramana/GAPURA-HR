...
export default function Edit({ auth, provider }) {
    const { data, setData, put, processing, errors, reset } = useForm({
        ...
        accreditation_expiry: provider.accreditation_expiry || '',
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        put(route('training-providers.update', provider.id), {
            preserveScroll: true,
        });
    };
    ...
    const getAccreditationWarning = () => {
        if (!data.accreditation_expiry) return null;

        const expiryDate = new Date(data.accreditation_expiry);
        const now = new Date();
        ...
        if (daysUntilExpiry < 0) {
            return {
                type: 'error',
                message: `Accreditation expired ${Math.abs(daysUntilExpiry)} days ago`
            };
    ...
    const accreditationWarning = getAccreditationWarning();
                ...
                <div className="max-w-4xl mx-auto sm:px-6 lg:px-8">
                    ...
                    {accreditationWarning && (
                        <div className={`mb-6 border rounded-lg p-4 ${
                            accreditationWarning.type === 'error'
                                ? 'bg-red-50 border-red-200'
                                : 'bg-yellow-50 border-yellow-200'
                        }`}>
                            <div className="flex">
                                <ExclamationTriangleIcon className={`h-5 w-5 ${
                                    accreditationWarning.type === 'error' ? 'text-red-400' : 'text-yellow-400'
                                }`} />
                                <div className="ml-3">
                                    <h3 className={`text-sm font-medium ${
                                        accreditationWarning.type === 'error' ? 'text-red-800' : 'text-yellow-800'
                                    }`}>
                                        Accreditation Alert
                                    </h3>
                                    <p className={`mt-1 text-sm ${
                                        accreditationWarning.type === 'error' ? 'text-red-700' : 'text-yellow-700'
                                    }`}>
                                        {accreditationWarning.message}
                                    </p>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="flex items-center justify-between">
                                <div className="flex items-center">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm text-gray-500">Update details for this training provider</p>
                                    </div>
                                </div>
                                    ...
                                    <div className="space-y-6">
                                        <div>
                                            ...
                                            <div className="mb-4">
                                                ...
                                                <p className="mt-1 text-xs text-gray-500">Rating from 0.0 to 5.0 based on performance</p>
                                            </div>
                                        ...
                                        <div>
                                            ...
                                            <div className="mb-4">
                                                ...
                                                <input
                                                    ...
                                                    className={`block w-full border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 sm:text-sm ${
                                                        accreditationWarning?.type === 'error' ? 'border-red-300' :
                                                        accreditationWarning?.type === 'warning' ? 'border-yellow-300' : ''
                                                    }`}
                                                />
                            ...
                            <div className="ml-3">
                                ...
                                <div className="mt-2 text-sm text-yellow-700">
                                    <ul className="list-disc list-inside space-y-1">
                                        <li><strong>Changing Status:</strong> Inactive providers won't appear in new training assignments</li>
                                        <li><strong>Rating Updates:</strong> Consider performance trends when adjusting ratings</li>
                                        <li><strong>Accreditation:</strong> Monitor expiry dates to ensure compliance</li>
                                        <li><strong>Contract Dates:</strong> Keep track of contract renewal dates</li>
                                    </ul>
                                </div>
                            </div>
                        </div>
